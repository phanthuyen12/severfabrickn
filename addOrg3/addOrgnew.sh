...
ORG_VALUE="${ORG_VALUE:-Org3}"
PORT_VALUE_CA="${PORT_VALUE_CA:-11054}"
. ../scripts/utils.sh
...
function generateOrg() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=${org_value}-crypto.yaml --output="../organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
...
function generateOrgDefinition() {
  ...
  export FABRIC_CFG_PATH=${PWD}/../../config
  docker-compose -f compose/compose-${org_value,,}.yaml -f compose/docker/docker-compose-${org_value,,}.yaml up -d
  docker ps
}
...
COMPOSE_FILE_BASE=compose/compose-${ORG_VALUE,,}.yaml
COMPOSE_FILE_ORG=compose/${CONTAINER_CLI}/docker-compose-${ORG_VALUE,,}.yaml
# certificate authorities compose file
COMPOSE_FILE_CA_BASE=compose/compose-ca-${ORG_VALUE,,}.yaml
COMPOSE_FILE_CA_ORG=compose/${CONTAINER_CLI}/docker-compose-ca-${ORG_VALUE,,}.yaml
# database
DATABASE="leveldb"
...
if [ "$MODE" == "up" ]; then
  infoln "Adding ${ORG_VALUE} to channel '${CHANNEL_NAME}' with '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE}'"
  echo
elif [ "$MODE" == "down" ]; then
  EXPMODE="Stopping network"
elif [ "$MODE" == "generate" ]; then
  EXPMODE="Generating certs and organization definition for ${ORG_VALUE}"
else
  printHelp
  exit 1
fi
